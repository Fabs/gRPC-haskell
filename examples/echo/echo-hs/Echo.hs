...
data Echo request response = Echo
  { echoDoEcho :: request 'HsGRPC.Normal Echo.EchoRequest Echo.EchoResponse -> Hs.IO (response 'HsGRPC.Normal Echo.EchoResponse)
  } deriving (Hs.Generic)
...
echoServer ::
  ...
  -> Hs.IO ()
echoServer Echo {echoDoEcho = echoDoEcho} (ServiceOptions serverHost serverPort useCompression userAgentPrefix userAgentSuffix initialMetadata sslConfig logger) =
  (HsGRPC.serverLoop
...
echoClient ::
     HsGRPC.Client -> Hs.IO (Echo HsGRPC.ClientRequest HsGRPC.ClientResult)
echoClient client =
  (Hs.pure Echo) <*>
  ((Hs.pure (HsGRPC.clientRequest client)) <*>
   (HsGRPC.clientRegisterMethod client (HsGRPC.MethodName "/echo.Echo/DoEcho")))

data EchoRequest = EchoRequest
  { echoRequestMessage :: Hs.Text
...
instance HsProtobuf.Message EchoRequest where
  encodeMessage _ EchoRequest {echoRequestMessage = echoRequestMessage} =
    (Hs.mconcat
...
instance HsProtobuf.Message EchoResponse where
  encodeMessage _ EchoResponse {echoResponseMessage = echoResponseMessage} =
    (Hs.mconcat
       [ (HsProtobuf.encodeMessageField
            (HsProtobuf.FieldNumber 1)
            echoResponseMessage)
